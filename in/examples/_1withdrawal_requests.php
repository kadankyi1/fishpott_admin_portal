...
$page_name_real = "current_withdrawals"; 
...
$query =  "SELECT " 
          ...
          . WITHDRAWAL_TABLE_NAME . ".action_admin,  " 
          . WITHDRAWAL_TABLE_NAME . ".paid_status,  " 
          . WITHDRAWAL_TABLE_NAME . ".paid_date,  " 
          . WITHDRAWAL_TABLE_NAME . ".settle_type,  " 
          . WITHDRAWAL_TABLE_NAME . ".pay_currency,  " 
          . WITHDRAWAL_TABLE_NAME . ".withdrawal_country,  " 
          ...
          . WITHDRAWAL_TABLE_NAME . ".old_withdraw_wallet_usd, "
          . WITHDRAWAL_TABLE_NAME . ".pay_amt_usd FROM "  
          . WITHDRAWAL_TABLE_NAME . " INNER JOIN " 
          . USER_BIO_TABLE_NAME . " ON  "  
          . WITHDRAWAL_TABLE_NAME . ".investor_id="  
...
if(isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == "POST" && !isset($_GET["o"]) ){
    ...
    if(isset($_POST["user_age_start"]) && trim($_POST["user_age_start"]) != ""){
        if(trim($_POST["user_age_start"]) == "all"){
            $_SESSION["current_withdrawals_where_clause_set_passive"] = true;
        } else {
            ...
            $where_clause_sql_query_addition .= " $or_where_and_statements " . WITHDRAWAL_TABLE_NAME . ".paid_status = ? ";
    ...
    $_SESSION["current_withdrawals_where_clause"] = $where_clause_sql_query_addition;
    $_SESSION["current_withdrawals_order_by_clause"] = $order_by_addition;
    $_SESSION["current_withdrawals_value_holder_array"] = $value_holder_array;
    $_SESSION["current_withdrawals_value_types_string"] = $value_types_string;

} else if(
    !isset($_GET["o"]) && 
    (

        (isset($_SESSION["current_withdrawals_where_clause_set_passive"]) && $_SESSION["current_withdrawals_where_clause_set_passive"]=== true) || 
        (isset($_SESSION["current_withdrawals_where_clause"]) && trim($_SESSION["current_withdrawals_where_clause"]) != "")
    )
){
    $value_holder_array = $_SESSION["current_withdrawals_value_holder_array"];
    $value_types_string = $_SESSION["current_withdrawals_value_types_string"];
    ...
    $query .= $_SESSION["current_withdrawals_where_clause"] . $_SESSION["current_withdrawals_order_by_clause"];
  ...
  if(isset($_GET["start_sku"]) && intval($_GET["start_sku"]) > 0){
    ...
    $where_clause_sql_query_addition =    " WHERE " . LOGIN_TABLE_NAME . ".flag = 0 AND " . WITHDRAWAL_TABLE_NAME . ".paid_status = 'pending' AND " . WITHDRAWAL_TABLE_NAME . ".action_admin = '' AND " . WITHDRAWAL_TABLE_NAME . ".sku > ? ";
    ...
    $_SESSION["current_withdrawals_order_by_clause"] = $order_by_addition;
        ...
        $where_clause_sql_query_addition =   " WHERE " . LOGIN_TABLE_NAME . ".flag = 0 AND " . WITHDRAWAL_TABLE_NAME . ".paid_status = 'pending' AND " . WITHDRAWAL_TABLE_NAME . ".action_admin = ''";
    }
        ...
        <div class="container-fluid">
            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header card-header-primary">
                  <h4 class="card-title">Search Withdrawal Requests</h4>
                  <?php if(isset($_SESSION["asem"]) && trim($_SESSION["asem"]) != "") { ?>
                  <p class="card-category"><?php echo $_SESSION["asem"]; unset($_SESSION["asem"]); ?></p>
                  <?php } else { ?>
                  <p class="card-category">Find a withdrawal requests  by setting the criteria you need</p>
                  <?php } ?>
                </div>
                <div class="card-body">
                  <form method="POST" action="_1withdrawal_requests.php">
                    <div class="row">
                      <div class="col-md-5">
                        <div class="form-group">
                          <select name="user_age_start" class="form-control">
                      ...
                      <div class="col-md-7">
                        <div class="form-group">
                          <label class="bmd-label-floating">Withdrawer Pott name</label>
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="card">
                <div class="card-header card-header-primary">
                  <h4 class="card-title ">Withdrawal Requests Table</h4>
                  <p class="card-category">Make sure the person requesting the for withdrawal has enough money in their withdrawal wallet to cover the request and confirm the request before and then send the money not the other way round</p>
                </div>
                <div class="card-body">
                  <div class="table-responsive">
                    <table class="table">
                      <thead class=" text-primary">
                        ...
                        <th style="color: #1f1f14; font-weight: bolder;">
                           PottName<br>- - - -<br>PayoutCountry
                        </th>
                        <th style="color: #1f1f14; font-weight: bolder;">
                           Old Withdrawal Wallet
                        </th>
                        <th style="color: #1f1f14; font-weight: bolder;">
                           Bank/Momo<br>- - - -<br>Type
                        </th>
                        <th style="color: #1f1f14; font-weight: bolder;">
                           Request-Date<br>- - - -<br>Pay-Date
                        </th>
                        <th style="color: #1f1f14; font-weight: bolder;">
                          Acc-Number
                        </th>
                    ...
                    ?>
                        ...
                        <tr>
                          ...
                          <td>
                           <span class="text-primary">@</span><b><?php echo $pot_name; ?></b><br>- - - -<br><?php echo $withdrawal_country; ?>
                          </td>
                          <td>
                           USD <?php echo $old_withdraw_wallet_usd; ?>
                          </td>
                          <td>
                           <?php echo $bank_network_name; ?><br>- - - -<br><?php echo $settle_type; ?>
                          </td>
                          <td>
                          <?php echo $request_date; ?><br>- - - -<br><?php echo $paid_date; ?>
                          </td>
                          <td class="text-primary">
                            <?php echo $account_number; ?>
                          </td>
                          <td class="text-primary">
                            <span class="text-primary"><?php echo $account_name; ?></span>
                          </td>
                          <td class="text-primary">
                            <?php echo $routing_number; ?>
                          </td>
                          <td class="text-primary">
                          <?php echo $pay_currency . " " .  $amount; ?><br>- - - -<br>USD <?php echo $pay_amt_usd; ?>
                          </td>
                          <td>
                            <?php if($paid_status == "pending" && $action_admin == "") { ?>
                            <?php if($bank_network_name == "Mobile Money[TAKEN OUT]"){ ?>
                            <a href="../../../inc/admin/withdrawal_request_send_momo.php?i=<?php echo $sku; ?>&t=1">
                                <img class="material-icons" src="../img/momo_transfer.png" alt="complete" style="width: 30px; height: 30px;">
                            </a>
                            <br>
                            <br>
                            <?php } ?>
                            <a href="../../../inc/admin/withdrawal_request_take_action.php?i=<?php echo $sku; ?>&t=1">
                                <img class="material-icons" src="../img/correct.png" alt="complete" style="width: 20px; height: 20px;">
                            </a>
                            <a href="../../../inc/admin/withdrawal_request_take_action.php?i=<?php echo $sku; ?>&t=0">
                    ...
                    $next_page_link = "_1withdrawal_requests.php?start_sku=" . $sku;
                    $previous_page_link = "_1withdrawal_requests.php?start_sku=" . $first_sku;
                    ?>
